...
blup_serial <- function(data, geno=NULL, what, index.coeff=NULL, gwas.ncore=0L) {
  ...
  # If no index is provided for harvest years, return id:harvest-year blups
  # First determine if the index refers to harvest years or locations
  #
  # Number of harvest years
  nhar <- length(index.scale) #number of loc/traits

  if (is.null(index.coeff)) {
    index.over.hars <- FALSE
    ...
    if (nhar > 1) {
      harvests <- names(index.scale)
      ix <- match(harvests, names(index.coeff))
  ...
  if (index.over.hars) {
    fix.value <- sum(data@avg.env * index.coeff)
  } else {
  ...
  if (what %in% c("AV","BV","GV")) {
    ...
    if (index.over.hars) {
      ...
      M <- kronecker(Diagonal( n = n.id), Diagonal(n = length(index.coeff), index.coeff))
      if (data@model > 1L)
        M <- cbind(M,gamma*M)
      out <- expand.grid(har = names(index.coeff), id = data@id, stringsAsFactors = FALSE)[,c(2,1)]
      out$value <- as.numeric(M %*% Matrix(data@random,ncol=1)) + fix.value
      numer <- diag(M%*% tcrossprod(data@var.uhat,M))
      denom <- diag(M%*% tcrossprod(data@var.u,M))
      out$r2 <- numer/denom
