...
blup_prep_serial <- function(data, vcov = NULL, geno = NULL, vars, mask = NULL, method = NULL) {
  ...
  if (!is.null(mask)) {
    tmp <- intersect(colnames(mask), c("trl", "id","har"))
  ...
  if (n.trait > 1) {
    data <- data[order(data$har,data$id,data$trait),]
    tmp <- paste(data$id,data$trait,sep=":")
  } else {
    data <- data[order(data$har,data$id),]
    tmp <- paste(data$id, data$har,sep=":")
  }
  if (!is.null(vcov)) {
    #stopifnot(nrow(vars@meanOmega) > 0)
    data$trl <- factor(data$trl, levels = names(vcov))
    omega.list <- mapply(FUN=function(Q, ix){
                          ix2 <- match(ix, rownames(Q))
                          as(Q[ix2,ix2,drop=FALSE],"dpoMatrix")
                        },Q = vcov,ix=split(tmp, data$trl))
    #names(omega.list) <- names(vcov)
  } else {
    data$trl <- factor(data$trl)
    omega.list <- lapply(split(tmp,data$trl),function(rnames){
                                n <- length(rnames)
  ...
  n.har.vcov <- length(hars)
  n.har <- length(unique(data$har))
  omega.list <- omega.list[trls]
  ...
  # Missing harvest years
  missing.har <- setdiff(hars, as.character(data$har))
  ...
  if (n.trait==1) {
    if (n.trl > 1) {
      X <- sparse.model.matrix(~trl + har - 1,data1,sep = "__")
      colnames(X) <- sub("trl__","",colnames(X),fixed=T)
      colnames(X) <- sub("har__","",colnames(X),fixed=T)
    } else {
      X <- sparse.model.matrix(~har - 1,data)
      colnames(X) <- hars
    }
    ...
    if (n.loc > 1) {
      ...
      Z <- sparse.model.matrix(~har:id-1, data, sep="__")
      colnames(Z) <- sub("har__","",colnames(Z),fixed=T)
      colnames(Z) <- sub("id__","",colnames(Z),fixed=T)

      # Reverse column names
      colnames(Z) <- sapply(X = lapply(X = strsplit(colnames(Z), ":"), FUN = rev), FUN = paste0, collapse = ":")
  ...
  # hars with data
  hars.with.data <- colSums(X) != 0

  if (n.loc > 1) {
    loc.har <- unique(data[,c("loc","har")])
    loc.har <- loc.har[order(loc.har$loc,loc.har$env),]
    ix <- match(names(fixed)[hars.with.data], as.character(loc.har$har))
    avg.har <- tapply(fixed[hars.with.data], loc.har$loc[ix], mean)
    tmp <- as.numeric(avg.har)
    names(tmp) <- names(avg.har)
    avg.har <- tmp
  } else {
    if (n.trait > 1) {
      ...
      # Single loc, single trait
      trl.har <- unique(data[,c("trl","har")])
      trl.har <- trl.har[order(trl.har$trl, trl.har$har),]
      ix <- match(names(fixed)[hars.with.data], as.character(trl.har$har))
      avg.har <- tapply(fixed[hars.with.data], trl.har$har[ix], mean)
      tmp <- as.numeric(avg.har)
      names(tmp) <- names(avg.har)
      avg.har <- tmp
    }
  ...
  if (length(missing.har)>0) {
    avg.har[missing.har] <- mean(avg.har, na.rm = T)
  }

  new(Class="class_prep",id=id,ploidy=ploidy,var.u=var.u,var.uhat=var.uhat,
      avg.env=avg.har,heterosis=heterosis,fixed.marker=fixed.marker,B=vars@B,
